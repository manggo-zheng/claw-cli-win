...
1. 启动openclaw后，有一定概率ws会将其检测为异常状态；
2. openclaw自重启后，有一定概率ws会将其检测为异常状态；
3. openclaw自重启后，有一定概率会跳出anpi的进程，独自开个CMD界面；
4. 重启选项存在问题，无法正常启动；
5. 当前的初始化逻辑存在问题，应该先握手测试，再进行启动。
5. 因为后续改用ws进行监控（为了满足后续的拓展），现阶段代码复杂度提升了一个量级，导致现阶段进展缓慢；
...
![ShareX_20260424_1825_207.png](test/ShareX_20260424_1825_207.png)
